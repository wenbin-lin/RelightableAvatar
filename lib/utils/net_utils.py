...
def load_geo_model(net, model_path):
    model = torch.load(model_path)['net']
    # net.load_state_dict(model)
    sdf = {k.split('network.')[1]: v for k, v in model.items() if 'sdf_network' in k}
    beta = {k.split('network.')[1]: v for k, v in model.items() if 'beta_network' in k}

    net.tpose_human.sdf_network.load_state_dict(sdf)
    net.tpose_human.beta_network.load_state_dict(beta)

    deform = {k.split('deform_net.')[1]: v for k, v in model.items() if 'deform_net' in k}
    net.deform_net.load_state_dict(deform)

    smpl_pose = {k.split('body_poses.')[1]: v for k, v in model.items() if 'body_poses' in k}
    net.body_poses.load_state_dict(smpl_pose)


def load_smpl_pose(net, model_path):
    model = torch.load(model_path)['net']
    smpl_pose = {k.split('body_poses.')[1]: v for k, v in model.items() if 'body_poses' in k}
    smpl_pose.update({k.split('betas.')[0]: v for k, v in model.items() if 'betas' in k})
    net.body_poses.load_state_dict(smpl_pose)


def load_vismodel(network, path):
    ckpt = torch.load(path, map_location=network.tpose_human.color_sg_network.envmap_material_network.lgtSGs.data.device)
    network.tpose_human.color_sg_network.visibility_network.load_state_dict(ckpt['model_state_dict'])
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True, only=[], skip=[]):
    if not resume:
        return 0
