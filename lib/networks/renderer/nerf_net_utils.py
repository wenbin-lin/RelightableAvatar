...
def rawsg2outputs(raw, z_vals, sg_values, sg_item_names, sg_map_names, white_bkgd=False):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        raw: [num_rays, num_samples along ray, 4]. Prediction from model.
        z_vals: [num_rays, num_samples along ray]. Integration time.
    ...
    ret = {}
    for i in range(len(sg_item_names)):
        sg_item = torch.sum(weights[..., None] * sg_values[sg_item_names[i]], -2)  # [N_rays, 3]
        if white_bkgd:
            sg_item = sg_item + (1. - acc_map[..., None])
        ret.update({sg_map_names[i]: sg_item})
